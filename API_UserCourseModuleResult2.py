...
with open(dir + 'user_course_module_result2.json', mode='w', encoding='utf-8') as f: json.dump([], f)


with open(dir + 'user_course2.json', 'r',  encoding='utf-8') as json_file:
    ...
    for index, records in enumerate(data):
        ...
        for d in json_decoded:
            for element in d.items():
                print(response, index, id_value1, id_value2, datetime.now(), 'user_course_module_result2')
            d['UserId'] = id_value1
            d['CourseId'] = id_value2
            d['LoadDateTime'] = datetime.now().isoformat()
            #jprint(json_decoded)
        with open(dir + 'user_course_module_result2.json', mode='a', encoding='utf-8') as fa:
            json.dump(json_decoded, fa, indent=4);

# fix it
    #FORMATTING
    with open(dir + 'user_course_module_result2.json', 'r') as json_file:
        formatted = re.sub(r'(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+', '\n', (json_file.read()[2:]).replace('][', '')).replace('    }\n    {', '    },\n    {')
    with open(dir + 'user_course_module_result2.json', mode='w', encoding='utf-8') as fa:
        fa.write(formatted)